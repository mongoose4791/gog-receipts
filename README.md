...
## Notes on receipt dates

The tool now prefers the purchase date provided by the Orders API (the `date` field on each order, a UNIX timestamp in seconds).
When available, this timestamp is converted to a stable `YYYY-MM-DD` string and used in the PDF filename. If the API date is
missing for any order, the tool falls back to parsing the date from the receipt preview page content (the bold text inside the
"Date of purchase" label).

## Notes on discovery and refactor

The module that saves receipts now derives receipt preview URLs exclusively from the GOG Orders API via fetchOrders().
The previous DOM-based discovery for preview links (collecting anchors from account pages) has been removed to avoid
duplicated behaviors and intermittent mismatches. Tests were updated accordingly to focus on the current flow.

Roadmap item update: "derive date from api" has been implemented as described above.
